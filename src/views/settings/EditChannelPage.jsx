...
const EditChannelPage = () => {
  ...
  useEffect(() => {
     homeApi.get(`/tv/channels/${channelId}`).then(result => {
      const { drm, ...channel } = result.data;
      const data = {
        ...channel,
        type: drm.type  || '',
        licenseUrl: drm.licenseUrl  || '',
        key_id: drm.clear_keys.key_id  || '',
        key: drm.clear_keys.key || '',
      }
      setFormState(data);
    })
  }, [])
  ...
  const handleSubmit = (event) => {
    ...
    homeApi.put(`/tv/channels/${formState.uid}`, body).then(result => navigate(-1));
  };
  ...
  return (
    <PageContainer title="React Home">
        <DashboardCard title="Editar Canal">
